...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12">
		<div class="panel">
			...
			<form method="post" action="<?php echo base_url() ?>index.php/productos/actualizar" autocomplete="off">
				<br>
						...
						<div class="col">
							...
							<select class="form-control" id="id_unidad" name="id_unidad" required>
								...
								<?php foreach ($unidades as $uni) { ?>
									<option value="<?php echo $uni->id; ?>" <?php if($dato->id_unidad == $uni->id) { echo 'selected'; } ?>><?php echo $uni->nombre; ?></option>
								<?php } ?>
							</select>
						</div>
						...
						<div class="col">	
							...
							<select class="form-control" id="id_categoria" name="id_categoria" required>
								...
								<?php foreach ($categorias as $cat) { ?>
									<option value="<?php echo $cat->id; ?>" <?php if($dato->id_categoria == $cat->id) { echo 'selected'; } ?>><?php echo $cat->nombre; ?></option>
								<?php } ?>
							</select>
						</div>
					</div>
					...
					<div class="row">
						<div class="col">
							...
							<select class="form-control" id="inventariable" name="inventariable" required >
								<option value="1" <?php if($dato->inventariable == 1) { echo 'selected'; } ?>>Si</option>
								<option value="0" <?php if($dato->inventariable == 0) { echo 'selected'; } ?>>No</option>
							</select>
						</div>
						
						<div class="col">
							<label for="precio_compra">Stock m&iacute;nimo</label>
							<input class="form-control" id="stock_minimo" type="number" min="0" name="stock_minimo" placeholder="Escribe aquí el stock m&iacute;nimo" value="<?php echo $dato->stock_minimo; ?>" <?php if($dato->inventariable == 0) { echo 'disabled'; } ?>>
						</div>
					</div>
				</div>
